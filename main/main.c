...
#define DEAD_RESTART                                                           \
  200 /* # frames to hold trigger after loss of life, to start next life */
...
void scheduleUnpackRoom() {
  ...
  startCharAnimation(TYPE_PILL1, AnimateBase[TYPE_PILL1] +
                                     9); // initial all-pill startup flash
}
...
void drawWord(const char *string, int y, int colour) {
  ...
  while ((ch = *string++) != NEW_LINE && ch) {
    ...
    if (ch != ' ') {
      ...
      drawBigDigit(ch, x, y + EXTERNAL(__sinoid)[(x + (frame >> 2)) & 15] - 2,
                   7);
      drawBigDigit(ch, x, y + EXTERNAL(__sinoid)[(x + (frame >> 2)) & 15] + 3,
                   0);
...
void swipeToPlayer() {
  ...
  if (swipeComplete) {
    ...
    switch (displayMode) {
    case DISPLAY_NORMAL:
      ypos = (manY + 1) * CHAR_HEIGHT / 3 -
             (scrollY[displayMode] >> SHIFT_SCROLLY) + 2;
...
#define TOOLONG 30
  ...
  if (
    ...
    if (!triggerNextLife /*&& manDead*/ &&
        triggerPressCounter >= DEAD_RESTART) {
