...
void unpackRoom(int *pX, int *pY, bool dryRun, int roomNumber) {
...
#define BOX '$'
	...
	unsigned char *p = room[roomNumber];
	while (*p) {
		...
		case PLAYERONGOAL:
			...
			// int screenTopEdgePixel = (scrollY[mode] >> SHIFT_SCROLLY) * 3;
			// int playerScreenY = playerShoeY - screenTopEdgePixel;

			// if (mode == DISPLAY_HALF) {
			// 	playerScreenX >>= 1;
			// 	playerScreenY = (playerScreenY * ((CHAR_HEIGHT_HALF << 16) / CHAR_HEIGHT)) >> 16;
			// }
